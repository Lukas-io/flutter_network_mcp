...
| `FLUTTER_NETWORK_MCP_CAPTURE_ALLOW` | — | comma-list | **Capture allowlist (0.9.13+).** Opt-in host or host/path globs (e.g. `api.example.com/stock/*`). When set, ONLY matching requests are persisted — everything else is dropped at capture time. For focused debugging. Complements the `ignored_hosts` denylist (which now also takes host/path globs); deny still wins inside the allowed set. Surfaced in `ignored_hosts action:list`. |
