...
| **RC3** | Per-session capture writers snapshot config (skiplist/allowlist) at attach; runtime "refresh" doesn't propagate to running writers. Proof: fresh attach under an active ignore captures 0 rows while a pre-existing session keeps capturing. | F20 (`ignored_hosts` no-op mid-session, `capture_allow` presumably too), the false "Capture writer refreshed" summary | Have writers read shared config per tick (or push updates); only then keep the "refreshed" message. |
